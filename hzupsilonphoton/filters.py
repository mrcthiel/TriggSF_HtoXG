...
def trigger_filter(evts: Events) -> Union[ArrayLike, ak.Array]:
    if evts.data_or_mc == "data":
        ...
        if evts.year == "2016":
            hlt_trigger_name = "IsoMu24"
        if evts.year == "2017":
            hlt_trigger_name = "IsoMu27"
        if evts.year == "2018":
            hlt_trigger_name = "IsoMu24"
...
def n_probe_muon_filter(evts: Events) -> ak.Array:
    return ak.num(evts.events.probe_muon) >= 2

def n_probe_photon_filter(evts: Events) -> ak.Array:
    return ak.num(evts.events.probe_photon) >= 1

def n_tag_muon_filter(evts: Events) -> ak.Array:
    return ak.num(evts.events.tag_muon) >= 1
